...
public class ItemListAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!RegExp.isValidExp(pn, REGEXP_NUMBER)) {
			...
			out.println("<script>location.href='/admin/itemList.do?pn=1';</script>");
			out.close();
			return null;
		}
		...
		if(page>pagenation.getTotalPageCount()) {
			response.setContentType("text/html;charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>location.href='/admin/itemList.do?pn="+pagenation.getTotalPageCount()+"';</script>");
			out.close();
			return null;
		}
