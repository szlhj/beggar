...
<a class="page_prev" href="/item/mainItem?pn=<%=pagenation.getStartPageNumber() - 1 %>&category=<%=category%>"> < </a>
<% } %>
...
<%if (pagenation.getTotalPageCount() != pagenation.getEndPageNumber()) {%>
		<a class="page_next" href="/item/mainItem?pn=<%=pagenation.getEndPageNumber() + 1 %>&category=<%=category%>"> > </a>
<% }%>
</div>
</body>
</html>