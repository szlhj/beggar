...
<link rel="stylesheet" href="/views/css/item/itemHome.css"
	type="text/css">
</head>
<script>
	function dummy() {
...
<body>
	<div class="row row-cols-1 row-cols-md-4 item_full">
		...
		<div class="col mb-3 item">
			<div class="card"
				onclick="location.href='/item/detail?item_sq=<%=list.get(i).getItem_sq()%>'">
				<img src="<%=list.get(i).getFilepath()%>" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title">
						이름<%=list.get(i).getItem_name()%></h5>
					<p class="card-text">
						가격<%=list.get(i).getPrice()%></p>
					<p class="card-text">
						할인가격<%=Parser.disPrice(list.get(i).getPrice(), list.get(i).getDiscount())%></p>
					<p class="card-text">
						할인율<%=list.get(i).getDiscount()%></p>
				</div>
			</div>
		</div>
		<%}	%>
